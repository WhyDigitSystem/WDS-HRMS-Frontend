...
import handleSampleFileSalaryStructure from '../../../src/assets/sample-files/OtherPaymentSample.xlsx';
...
const OtherPayments = () => {
  ...
  const getAllOtherPayments = async () => {
    try {
      const month = monthYear ? monthYear.getMonth() + 1 : null;
      const year = monthYear ? monthYear.getFullYear() : '';

      const response = await apiCalls('get', `/checkinout/getAllOtherPaymentsByOrgId?orgId=${orgId}&month=${month}&year=${year}`);
      console.log('API Response:', response);
      setListViewData(response.paramObjectsMap.otherPaymentsVO || []);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      // const formattedDate = getFormattedMonthYear(monthYear);
      const month = monthYear ? monthYear.getMonth() + 1 : null;
      const year = monthYear ? monthYear.getFullYear() : '';
      ...
      try {
        const response = await apiCalls('put', `/checkinout/createUpdateOtherPayments`, saveFormData);
        if (response.status === true) {
          showToast('success', editId ? 'Other Payment Updated Successfully' : 'Other Payment created successfully');
          getAllOtherPayments();
          handleClear();
        } else {
          showToast('error', response.paramObjectsMap.errorMessage || 'Other Paymnet creation failed');
        }
      } catch (error) {
        console.error('Error:', error);
        showToast('error', 'Other Payment creation failed');
      } finally {
        setIsLoading(false);
      }
    } else {
  ...
  const handleListViewSubmit = async () => {
    ...
    try {
      ...
      await getAllOtherPayments(); // ✅ call API with selected monthYear
        ...
        <div className="row d-flex ml">
          <div className="d-flex flex-wrap justify-content-start mb-4" style={{ marginBottom: '20px' }}>
            ...
            <ActionButton
              ...
              onClick={() => {
                setUploadFile({
                  title: 'Upload Other Payment',
                  apiUrl: '/checkinout/uploadOtherPaymentsExcel',
                  sampleFileDownload: handleSampleFileSalaryStructure,
                  sampleFileName: 'Other Payment Sample File',
                    ...
                    {filteredData.map((advance) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>{formatNumberWithCommas(advance.amount)}</TableCell>
