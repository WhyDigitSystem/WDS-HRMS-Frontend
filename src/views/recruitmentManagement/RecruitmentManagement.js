...
import { createTheme, Box, AppBar, Toolbar, Button, Tabs, Tab, Container, Chip } from '@mui/material';
...
const RecruitmentManagement = () => {
  ...
  const initializeApp = async () => {
    try {
      ...
      await Promise.all([fetchJobs(), fetchCandidates(), fetchInterviews(), fetchOffers()]);
    } catch (error) {
  ...
  const fetchInterviews = async () => {
    try {
      ...
      if (response.status === true) {
        ...
        const interviewsData = response.paramObjectsMap?.candidatesVO || [];
  ...
  const fetchOffers = async () => {
    try {
      const response = await apiCalls(
        'get',
        `recruitmentmanagement/getCreateOfferByOrgIdAndDepartment?orgId=${orgId}&branchCode=${branchCode}&department=ALL&status=ALL`
      );
  ...
  return (
    ...
    >
      <AppBar
        ...
        sx={{
          background: '#ffffff',
          borderBottom: '1px solid #dbe5e6',
          boxShadow: '0 4px 12px rgba(58,107,109,0.08)'
        }}
      >
        <Toolbar
          disableGutters
      ...
      <Container component="main" sx={{ flexGrow: 1, py: 2 }} maxWidth="xl">
        ...
        {currentTab === 'offers' && (
          <Offers offers={offers} onOpenModal={openModal} onCloseModal={closeModal} config={config} onRefresh={fetchOffers} />
        )}
      </Container>

      {/* Reusable Modal */}
      <ReusableModal open={modalOpen} onClose={closeModal} config={config} {...modalConfig} />
    </Box>
  );
};
