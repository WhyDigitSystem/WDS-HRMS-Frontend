...
const CreateOffer = () => {
    ...
    const handleSubmit = async (isDraft = false) => {
        ...
        try {
            const response = await apiCalls(
                'put',
                'recruitmentmanagement/createUpdateCreateOffer',
                apiData
            );
            ...
            >
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={4}>
                            <Autocomplete
                                ...
                                value={
                                    candidates.find((c) => c.candidatesName === formData.candidateName) || null
                                }
                            ...
                            <Autocomplete
                                ...
                                value={
                                    allReportingPersonList.find(
                                        (emp) => emp.label === formData.reportingTo
                                    ) || null
                                }
                                            ...
                                            >
                                                {salaryHeadsType
                                                    .filter(
                                                        (head) =>
                                                            !compensations.some(
                                                                (comp, i) => comp.type === head.heading && i !== index
                                                            )
                                                    )
                                                    .map((head) => (
                                                        <MenuItem
                ...
                <Box sx={{ mb: 4 }}>
                    <Typography
                        variant="h5"
                        gutterBottom
                        sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2, fontWeight: 'bold', color: 'primary.main' }}
                    >
