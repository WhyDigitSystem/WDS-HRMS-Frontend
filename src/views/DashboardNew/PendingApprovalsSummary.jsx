...
const PendingApprovalsSummary = () => {
        ...
        try {
            ...
            if (userType === "ADMIN") {
                const [
                    ...
                    apiCalls('get', `incrementmanagement/getIncrementManagementForDashBoard?branchCode=${branchCode}&orgId=${orgId}&reportingPersonCode=${employeeCode}`),
                    apiCalls('get', `/assetmanagement/getExpenseClaimsForDashBoard?branchCode=${branchCode}&orgId=${orgId}&reportingPersonCode=${employeeCode}`),
                    apiCalls('get', `/assetmanagement/getTravelRequestsForDashBoard?branchCode=${branchCode}&orgId=${orgId}&reportingPersonCode=${employeeCode}`)
                ]);
    ...
    const handleActionLeave = async (request, action) => {
        ...
        try {
            await apiCalls('put', `/leaveprocess/createApprovalLeave?action=${action}&actionBy=${loginUserName}&employeeCode=${request.employeeCode}&id=${request.id}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}&email=${request.email}`);
    ...
    const handleActionPermission = async (request, action) => {
        ...
        try {
            await apiCalls('put', `/employeemaster/createApprovalPermissionRequest?action=${action}&actionBy=${loginUserName}&employeeCode=${request.employeeCode}&id=${request.permissionRequestId || request.id}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}`);
    ...
    const handleActionCompoOff = async (request, action) => {
        setProcessingId(request.id);
        try {
            await apiCalls('put', `/leaveprocess/createApprovalCompOff?action=${action}&actionBy=${loginUserName}&employeeCode=${request.employeeCode}&id=${request.id}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}`);
    ...
    const handleActionCheckout = async (request, action) => {
        setProcessingId(request.id);
        try {
            await apiCalls('put', `/basicmaster/createApprovalCheckOut?action=${action}&actionBy=${loginUserName}&employeeCode=${request.employeeCode}&checkOutDate=${request.checkInDate}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}`);
    ...
    const handleCheckInOutApprove = async (request, action) => {
        setProcessingId(request.id);
        try {
            const response = await apiCalls('put', `/basicmaster/createApprovalCheckInOutAdjustment?action=${action}&actionBy=${loginUserName}&employeeCode=${request.employeeCode}&checkOutDate=${request.checkInDate}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}`);
    ...
    const handleActionIncrementManagement = async (request, action) => {
        setProcessingId(request.id);
        try {
            const response = await apiCalls('put', `/incrementmanagement/createApprovalIncrementManagement?action=${action}&actionBy=${employeeName}&employeeCode=${request.employeeCode}&id=${request.id}&orgId=${orgId}&notifyCode=${employeeCode}&notify=${employeeName}&screenName=${request.screenName}`);
    ...
    const handleActionExpense = async (request, action) => {
        setProcessingId(request.id);
        try {
            const response = await apiCalls('put', `/assetmanagement/createApprovalExpenseClaims?action=${action}&actionBy=${employeeName}&employeeCode=${request.employeeCode}&id=${request.id}&notify=${employeeCode}&notifyCode=${employeeCode}&orgId=${orgId}&screenName=${request.screenName}&approvedAmount=${request.amount}`);
    ...
    const handleActionTravel = async (request, action) => {
        setProcessingId(request.id);
        try {
            const response = await apiCalls('put', `/assetmanagement/createApprovalTravelRequests?action=${action}&actionBy=${employeeName}&employeeCode=${request.employeeCode}&id=${request.id}&notify=${employeeCode}&notifyCode=${employeeCode}&orgId=${orgId}&screenName=${request.screenName}&approvedAmount=${request.estimatedCost}`);
    ...
    const ActionButtons = ({ request }) => {
        ...
        const getActionHandler = (action) => {
            switch (request.screenName) {
                case 'LEAVE REQUEST': return () => handleActionLeave(request, action);
                case 'PERMISSION REQUEST': return () => handleActionPermission(request, action);
                case 'COMPENSATORY OFF': return () => handleActionCompoOff(request, action);
                case 'CHECKINOUT': return () => handleActionCheckout(request, action);
                case 'CHECKINOUTADJUSTMENT': return () => handleCheckInOutApprove(request, action);
                case 'EXPENSE CLAIMS': return () => handleActionExpense(request, action);
                case 'TRAVEL REQUEST': return () => handleActionTravel(request, action);
                default: return () => { };
            }
        };
        ...
        return (
            <Box display="flex" gap={1}>
                <Tooltip title="Approve">
                    <IconButtonStyled
                        actiontype="approve"
                        onClick={getActionHandler('APPROVED')}
                ...
                <Tooltip title="Reject">
                    <IconButtonStyled
                        actiontype="reject"
                        onClick={getActionHandler('REJECTED')}
                        disabled={isProcessing}
                    >
            ...
            return (
                <Box display="grid" gridTemplateColumns="repeat(4, 1fr)" gap={4}>
                    ...
                    <Box>
                        ...
                        <Typography sx={{ fontWeight: 700, fontSize: '0.88rem', color: '#334155' }}>{dayjs(request.startDate).format('DD-MM-YYYY')}</Typography>
                    </Box>
                    <Box>
                        <Typography sx={{ fontSize: '0.72rem', color: '#7b8794', mb: 0.5, textTransform: 'uppercase' }}>To Date</Typography>
                        <Typography sx={{ fontWeight: 700, fontSize: '0.88rem', color: '#334155' }}>{dayjs(request.endDate).format('DD-MM-YYYY')}</Typography>
                    </Box>
                </Box>
            );
        } else if (request.screenName === 'PERMISSION REQUEST') {
            ...
            >
                ...
                {loading ? (
                    ...
                    <Box textAlign="center" py={2}>
                        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.75rem' }}>No pending requests</Typography>
                    </Box>
                )}
            </Paper>
                ...
                <Box sx={{ px: 3, py: 2, background: 'linear-gradient(90deg, #1e3a5f 0%, #294d73 100%)', borderBottom: '1px solid #dbe3ec' }}>
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                        ...
                        <Box display="flex" alignItems="center" gap={1}>
                            ...
                            <IconButton onClick={handleCloseDialog} sx={{ bgcolor: 'rgba(255,255,255,0.08)', color: '#ffffff', border: '1px solid rgba(255,255,255,0.12)', '&:hover': { bgcolor: 'rgba(255,255,255,0.16)' } }}>
                ...
                <DialogContent sx={{ p: 2.5, bgcolor: '#f4f7fb' }}>
                    {groupedRequests.map((category, catIdx) => (
                        <Box key={catIdx} sx={{ mb: 3 }}>
                            <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 1.5, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Box sx={{ width: 4, height: 20, bgcolor: '#2196f3', borderRadius: 2 }} />
                                {category.type} ({category.count})
                            </Typography>
                            <List sx={{ p: 0 }}>
                                {category.data.map((request, idx) => (
                                    <ListItem key={idx} sx={{ p: 2.2, mb: 2, borderRadius: '18px', bgcolor: '#ffffff', border: '1px solid #dbe3ec', display: 'grid', gridTemplateColumns: '280px 1fr 120px', alignItems: 'center', gap: 3, transition: '0.25s ease', '&:hover': { boxShadow: '0 8px 24px rgba(30,58,95,0.08)', borderColor: '#bfd0e2' } }}>
                                        <Box display="flex" alignItems="center" gap={1.5}>
                                            <Avatar sx={{ width: 58, height: 58, bgcolor: '#2f5d8a', fontWeight: 700, fontSize: '1rem', boxShadow: '0 4px 10px rgba(47,93,138,0.18)' }}>
                                                {request.employeeName?.charAt(0) || 'U'}
                                            </Avatar>
                                            <Box>
                                                <Typography sx={{ fontWeight: 700, fontSize: '0.94rem', color: '#1e293b' }}>{request.employeeName || 'Unknown'}</Typography>
                                                <Typography sx={{ fontSize: '0.74rem', color: '#64748b', mt: 0.4 }}>Employee ID - {request.employeeCode || '-'}</Typography>
                                            </Box>
                                        </Box>
