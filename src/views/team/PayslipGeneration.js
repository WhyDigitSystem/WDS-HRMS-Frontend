...
const PayslipGeneration = () => {
  ...
  const handleSearch = () => {
    ...
    fetchPayslipData(employeeCode);
    setShowPayslip(true);
    setOpenPayslipDialog(true);
  };

  const handleClear = () => {
    // Reset to current month and year instead of previous month
    const now = dayjs();
  ...
  // Update the disableFutureMonth function to allow current month
  const disableFutureMonth = (month) => {
    const currentYear = dayjs().year();
    const currentMonth = dayjs().month(); // 0-indexed
    const selectedYearNum = Number(selectedYear);
    ...
    if (selectedYearNum === currentYear) {
      // Allow current month, disable only future months
      return month.month() > currentMonth;
    }
        ...
        <div className="col-md-3 mb-3">
          <Autocomplete
            ...
            onChange={(event, newValue) => {
              ...
              if (newValue) {
                setEmployeeCode(newValue.employeeCode);
              } else {
                setEmployeeCode('');
              }
              setShowPayslip(false);
